document.addEventListener('DOMContentLoaded', function() {
    ...
    /**
     * Mengambil data pembayaran dari Google Sheet (CSV).
    ...
    async function fetchPaymentData() {
        try {
            ...
            console.log("Data berhasil dimuat:", allPaymentData);
        } catch (error) {
            console.error('Error fetching data:', error);
            noResultsMessage.textContent = 'Gagal memuat data pembayaran. Silakan cek koneksi internet atau URL Google Sheet.';
    ...
    /**
     * Mengurai teks CSV menjadi array objek JavaScript.
     * @param {string} csvText - Teks CSV mentah.
     * @returns {Array<Object>} - Array objek data pembayaran.
     */
    function parseCSV(csvText) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (values.length !== headers.length) {
                console.warn(`Baris ${i + 1} dilewati karena format tidak valid.`);
                continue;
            }
    ...
    /**
     * Mengembalikan nama kelas CSS berdasarkan status pembayaran.
     * @param {string} statusText - Teks status pembayaran (misalnya "Lunas").
     ...
     * @param {number|string} amount - Jumlah nominal.
    ...
    /**
     * Menampilkan daftar pembayaran manual yang telah ditambahkan.
     */
    ...
    /**
     * Menampilkan detail siswa dan daftar pembayarannya.
     * @param {Object|null} studentData - Objek data siswa atau null jika tidak ditemukan.
     */
    function displayStudentDetails(studentData) {
        currentStudentData = studentData;
        manualArrears = []; // Reset pembayaran manual setiap kali ganti siswa
    ...
    /**
     * Mencari data siswa berdasarkan nama siswa/NISN dan nama orang tua.
     */
    ...
    /**
     * Menghasilkan dan mencetak slip pembayaran.
     * @param {Object} studentData - Data siswa.
     * @param {Array<Object>} selectedPayments - Array pembayaran yang dipilih.
     */
    ...
    /**
     * Menghasilkan dan mencetak tagihan tunggakan pembayaran.
     * @param {Object} studentData - Data siswa.
     */
    function generateCombinedArrearsSlip(studentData) {
        ...
        paymentColumns.forEach(colName => {
            ...
            if (status && (status.toLowerCase().trim() === 'belum lunas' || status.toLowerCase().trim() === 'tertunda')) {
                ...
                let nominalValue = nominalInput ? parseInt(nominalInput.value) || 0 : 0;
                if (colName.startsWith('Status_SPP_') && nominalValue === 0) {
                     nominalValue = SPP_NOMINAL_AMOUNT; // Gunakan nominal SPP default jika input kosong
                }

                let displayColName = colName.startsWith('Status_SPP_') ?
                    `SPP ${colName.replace('Status_SPP_', '')}`.replace(/_/g, ' ') :
                    colName.replace(/_/g, ' ');

                arrearsItems.push({ name: displayColName, nominal: nominalValue });
    ...
    printSlipButton.addEventListener('click', function() {
        ...
        // Ambil pembayaran yang dicentang dari Google Sheet
        checkboxes.forEach(checkbox => {
            ...
            const nominalValue = nominalInput ? parseInt(nominalInput.value) || 0 : 0;
        ...
        manualArrears.forEach(item => {
            selectedPayments.push({
                colName: item.name,
                status: 'Lunas', // Asumsi pembayaran manual adalah lunas
